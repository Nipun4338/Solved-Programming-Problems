...
vector<ll> constructTempArray(string pattern)
{
    ...
    ll index = 0;
    for (int i = 1; i < (ll) pattern.length(); )
    {
...
int main()
{
    ...
    while(testcase--)
    {
        ...
        vector<ll> lps=constructTempArray(text);
        if(lps[text.size()-1]!=0)
        {
            printf("%lld\n",(text.size()*b)-(--b*lps[text.size()-1]));
        }
        else
        {
            printf("%lld\n",text.size()*b);
...
/// for more info see problem + solution - https://codeforces.com/contest/1326/problem/D2 and
/// video of channel- kmp details - Loveextendscode
