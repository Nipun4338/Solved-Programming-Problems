...
int main()
{
    ...
    ll a;
    double ab,ac,bc,ratio1,check=0,ad;
    cin>>a;
    for(int i=0; i<a; i++)
    {
        cin>>ab>>ac>>bc>>ratio1;
        ...
        while(true)
        {
            double ae=(ad*ac)/ab;
            double de=(ad*bc)/ab;
            double s=0.5*(ad+ae+de);
            double triangle=sqrt(s*(s-ad)*(s-ae)*(s-de));
            double s1=0.5*((ab-ad)+(ac-ae)+de+bc);
            double trapezium=((de+bc)/abs((bc-de)))*sqrt((s1-de)*(s1-bc)*(s1-bc-(ab-ad))*(s1-bc-(ac-ae)));
            check=triangle/trapezium;
            //cout<<ad<<" "<<trapezium<<" "<<check<<endl;
            check=round(check*10000000000.0)/10000000000.0;
            if(check==ratio1)
            {
                break;
            }
