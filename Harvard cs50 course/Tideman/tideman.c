...
bool vote(int rank, string name, int ranks[])  // update per voter
{
    // TODO
    for (int i = 0; i < candidate_count; i++)
    {
...
void sort_pairs(void)
{
    ...
    for (int n = 0; n < pair_count; ++n)
    {
        for (int i = 0; i < (pair_count - 1); ++i)
        {
            ...
            {
                int x =  pairs[i].winner, y = pairs[i].loser;
                pairs[i] = pairs[i + 1];
                pairs[i + 1].winner = x;
                pairs[i + 1].loser = y;
            }
...
bool Cycle(int start, int end, int cycle_start);
...
// Returns true if it forms a cycle
bool Cycle(int start, int end, int cycle_start)
{
    if (end == cycle_start)
    {
